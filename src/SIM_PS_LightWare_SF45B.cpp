...
void PS_LightWare_SF45B::update_input()
{
    ...
    if (n < 0) {
        ...
        if (errno != EAGAIN && errno != EWOULDBLOCK && errno != 0) {
            ::fprintf(stderr, "Failed to read from autopilot");
        }
    } else {
        _buflen += n;
    }
    ...
    case InputState::WANT_CRC: {
        ...
        if (calc_checksum == got_checksum) {
            ...
            ::fprintf(stderr, "sim-SF45B: Bad checksum");
        }
        // consume these bytes
...
void PS_LightWare_SF45B::update()
{
    update_input();
    update_output();
}
...
//         float distance = 30; // Old method: distance in meters
...
ssize_t PS_LightWare_SF45B::write_to_autopilot(const char *buffer, const size_t size)
{
    ssize_t ret = 0;
    return ret;
}; 
...
ssize_t PS_LightWare_SF45B::read_from_autopilot(char *buffer, const size_t size)
{
    ssize_t ret = 0;
    return ret;
}; 
